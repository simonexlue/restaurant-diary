...
export default function MapFriendCard({ friend, isSelected = false, onClick }) {
    ...
    return (
        <div
            onClick={onClick}
            className={`flex flex-row items-center justify-between hover:bg-[rgb(245,232,214)] rounded-lg px-2 py-2 ${isSelected ? "bg-[rgb(245,232,214)]" : ""} ${onClick ? "cursor-pointer" : ""}`}
        >
            ...
            {onClick && (
                <FiChevronRight className="text-[rgb(137,122,114)]" size={18} />
            )}
        </div>
    );
}